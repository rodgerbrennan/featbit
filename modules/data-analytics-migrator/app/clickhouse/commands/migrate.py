...
def migrate(upto: int = 9999, check: bool = False, plan: bool = False, print_sql: bool = False) -> None:
    ...
    logger.info("Migration in ClickHouse")

    if check or plan:
        if print_sql:
            logger.info("List of clickhouse migrations to be applied:")
        migrations = list(_get_unapplied_migrations(database, upto))
        for migration_name, operations in migrations:
            logger.info(f"Migration would get applied: {migration_name}")
            for op in operations:
                sql = getattr(op, "_sql", None)
                if print_sql and sql is not None:
                    logger.info(indent("\n\n".join(sql), "    "))
        if len(migrations) == 0:
            logger.info("Clickhouse migrations up to date!")
        else:
            exit(1)
    else:
