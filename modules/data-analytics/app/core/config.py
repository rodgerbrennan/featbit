...
class Settings(BaseSettings):
    ...
    IS_PRO: str =  get_from_env("IS_PRO", False, type_cast=str_to_bool)
    TEST: str = get_from_env("TEST", True, type_cast=str_to_bool)
    SUFFIX: str = os.getenv("SUFFIX", "")

    KAFKA_HOSTS: str = os.getenv("KAFKA_HOSTS", "localhost:29092")
    KAFKA_SECURITY_PROTOCOL: str | None = os.getenv("KAFKA_SECURITY_PROTOCOL", None)
    KAFKA_SASL_MECHANISM: str | None = os.getenv("KAFKA_SASL_MECHANISM", None)
    KAFKA_SASL_USER: str | None = os.getenv("KAFKA_SASL_USER", None)
    KAFKA_SASL_PASSWORD: str | None = os.getenv("KAFKA_SASL_PASSWORD", None)
    KAFKA_PRODUCER_RETRIES: int = 3
    KAFKA_PRODUCER_ENABLED: str = get_from_env("KAFKA_PRODUCER_ENABLED", True, type_cast=str_to_bool)
    KAFKA_PREFIX: str = os.getenv("KAFKA_PREFIX", "")

    CLICKHOUSE_HOST: str = os.getenv("CLICKHOUSE_HOST", "localhost")
    CLICKHOUSE_ALT_HOST: str | None = os.getenv("CLICKHOUSE_ALT_HOST", None)
    ...
    CLICKHOUSE_SECURE: str = get_from_env("CLICKHOUSE_SECURE", False, type_cast=str_to_bool)
    CLICKHOUSE_USER: str = os.getenv("CLICKHOUSE_USER", "default")
    CLICKHOUSE_PASSWORD: str = os.getenv("CLICKHOUSE_PASSWORD", "")
    CLICKHOUSE_CA: str | None = os.getenv("CLICKHOUSE_CA", None)
    CLICKHOUSE_VERIFY: str = get_from_env("CLICKHOUSE_VERIFY", True, type_cast=str_to_bool)
    ...
    CLICKHOUSE_ENABLE_STORAGE_POLICY: str = get_from_env("CLICKHOUSE_ENABLE_STORAGE_POLICY", False, type_cast=str_to_bool)
    CLICKHOUSE_KAFKA_HOSTS: str = os.getenv("CLICKHOUSE_KAFKA_HOSTS", "kafka:9092")
    CLICKHOUSE_REPLICATION: str = get_from_env("CLICKHOUSE_REPLICATION", True, type_cast=str_to_bool)

    CACHE_TYPE: str = os.getenv("CACHE_TYPE", "RedisCache")
    CACHE_KEY_PREFIX: str = "da-server"
    REDIS_USER: str | None = os.getenv("REDIS_USER", None)
    REDIS_PASSWORD: str | None = os.getenv("REDIS_PASSWORD", None)
    REDIS_DB: int = get_from_env("REDIS_DB", 0, type_cast=int)
    REDIS_SSL: str = get_from_env("REDIS_SSL", False, type_cast=str_to_bool)
    ...
    REDIS_SENTINEL_PASSWORD: str | None = os.getenv("REDIS_SENTINEL_PASSWORD", None)
